...
function applyPreset(preset) {
  ...
  if (preset.linein) {
    ...
    let targetplayer = this.getPlayer(preset.linein);

    // Create line-in 'rincon-stream' uri from UUID
    var lineuri = 'x-rincon-stream:'+targetplayer.uuid;

    logger.debug('Selecting Line-In from '+preset.linein+' --> '+lineuri);

    // Set preset uri accordingly, let the following block do the dirty work of setting the source for the group
    preset.uri = lineuri;
  }
