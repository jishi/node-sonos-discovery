...
function parseTrackMetadata(metadata, nextTrack) {
  return new Promise((resolve, reject) => {
    ...
    sax.on('tag:item', (item) => {
      ...
      track.duration = parseTime(item.res.$attrs.duration);
...
function Player(data, listener, system) {
  ...
  _this.dispose = function dispose() {
    ...
    if (avTransportSubscriber) {
      avTransportSubscriber.dispose();
    }
  };

  function handleAVTransportSubscription(uuid, topology) {
    const isCoordinator = topology.some(zone => zone.$attrs.coordinator === _this.uuid);

    if (isCoordinator && !avTransportSubscriber) {
      avTransportSubscriber = new Subscriber(
        `${_this.baseUrl}/MediaRenderer/AVTransport/Event`,
        listener.endpoint()
      );
    } else if (!isCoordinator && avTransportSubscriber) {
      avTransportSubscriber.dispose();
      avTransportSubscriber = null;
    }
  }
  ...
  function notificationHandler(uuid, data) {
    ...
    if (data.transportstate) {
      ...
      parseTrackMetadata(data.currenttrackmetadata)
        ...
        .then(() => {
          if (!_this.avTransportUri.startsWith('x-rincon:')) {
  ...
  listener.on('topology', handleAVTransportSubscription);
}
