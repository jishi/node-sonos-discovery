...
function getPlayMode(state) {
  let key = state.shuffle << 1 | (state.repeat > 1 ? 4 : state.repeat);
...
function Player(data, listener, system) {
  ...
  function notificationHandler(uuid, data) {
    ...
    if (data.transportstate) {
      ...
      state.playMode.repeat = !!(currentPlayMode & PLAY_MODE.REPEAT_ALL) ? 1 : !!(currentPlayMode & PLAY_MODE.REPEAT_ONE) ? 2 : 0;
      state.playMode.shuffle = !!(currentPlayMode & PLAY_MODE.SHUFFLE_NOREPEAT);
...
Player.prototype.repeat = function repeat(mode) {
  ...
  logger.debug(`Setting repeat to ${playMode}`);
  return soap.invoke(
    `${this.baseUrl}/MediaRenderer/AVTransport/Control`,
    TYPE.SetPlayMode,
    {
