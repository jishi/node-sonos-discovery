...
Player.prototype.addURIToSavedQueue = function addURIToSavedQueue(sqid, uri, title) {
  // TODO do not prefix uris with x-file-cifs. use the raw export uri, but strip the cifs part for the DIDL cdata.
  console.log('xxxxxxxxxxxxxx addURIToSavedQueue:', sqid + ',' + uri + ',' + title);
  let upnpClass = 'object.item.audioItem.musicTrack';
  if (uri.indexOf('savedqueues.rsq') !== -1) {
    upnpClass = 'object.container.playlistContainer';
  }

  let shortURI = ''; // remove x-file-cifs://
  return this.browse('SQ:' + sqid, 0, 100).then((res) => {
        console.log('xxxxxxxxxxxxxx addURIToSavedQueue res', res);
        let updateID = res.updateID;
        return soap.invoke(
          `${this.baseUrl}/MediaRenderer/AVTransport/Control`,
  ...
  return soap.invoke(
    ...
    .then(res => {
      return new Promise((resolve, reject) => {
        ...
        console.log('xxxxxxxxxxxxx1', returnResult);
