...
function getState(playerInternal, coordinatorInternal, sub, surround) {
  var diff = 0;
...
function Player(data, listener, system) {
  ...
  function notificationHandler(uuid, data) {
    ...
    if (data.volume) {
      ...
      let LF = data.volume.find(x => x.channel === 'LF');
      let RF = data.volume.find(x => x.channel === 'RF');
      const previousVolume = state.volume;
      state.volume = parseInt(master.val);
      state.volumeLeft = parseInt(LF.val);
      state.volumeRight = parseInt(RF.val);

      let level = 0;
      if (state.volumeLeft < 100) {
        level = 100 - state.volumeLeft;
      } else if (state.volumeRight < 100) {
        level = state.volumeRight - 100;
      }

      _this._setBalance(level);
    ...
    if (data.surroundenabled) {
      _this._state.equalizer.surroundEnabled = data.surroundenabled.val === '1';
    }

    if (data.surroundmode) {
      _this._state.equalizer.surroundMode = data.surroundmode.val;
    }

    if (data.surroundlevel) {
      _this._state.equalizer.surroundLevel = parseInt(data.surroundlevel.val);
    }

    if (data.musicsurroundlevel) {
      _this._state.equalizer.musicSurroundLevel = parseInt(data.musicsurroundlevel.val);
    }
...
Player.prototype.setVolume = function setVolume(level, channel) {
  ...
  switch (channel) {
    case 'LF':
      ...
      if (level < 100) {
        this._setVolume(100, 'RF');
        soap.invoke(
          `${this.baseUrl}/MediaRenderer/RenderingControl/Control`,
          TYPE.Volume,
          { channel: 'RF', volume: 100 });
        balance = 100 - level;
      }
    ...
    case 'RF':
      ...
      if (level < 100) {
        this._setVolume(100, 'LF');
        soap.invoke(
          `${this.baseUrl}/MediaRenderer/RenderingControl/Control`,
          TYPE.Volume,
          { channel: 'LF', volume: 100 });
        balance = level - 100;
      }
...
Player.prototype.setLoudness = function setLoudness(enabled) {
  return soap.invoke(
    `${this.baseUrl}/MediaRenderer/RenderingControl/Control`,
    TYPE.SetLoudness,
    { loudness: Boolean(enabled) });
};
