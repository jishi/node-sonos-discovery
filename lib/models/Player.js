...
function getState(internal) {
  var diff = 0;
  if (internal.currentState === 'PLAYING')
    diff = new Date().valueOf() - internal.stateTime;

  var elapsedTime = internal.relTime + Math.floor(diff / 1000);

  return {
    currentTrack: internal.currentTrack,
    nextTrack: internal.nextTrack,
    volume: internal.volume,
    mute: internal.mute,
    trackNo: internal.trackNo,
    elapsedTime: elapsedTime,
    elapsedTimeFormatted: formatTime(elapsedTime),
    playbackState: internal.currentState,
    playMode: internal.playMode
  };
}
...
Player.prototype.setAVTransport = function setAVTransport(uri, metadata) {
  ...
  return soap.invoke(
    ...
    { uri, metadata: entityEncodedMetadata });
};
