...
function Player(roomName, descriptorUrl, uuid, discovery) {
  ...
  function handleNotification(notification) {
    ...
    if (notification.type == 'GroupVolume') {
      _this.groupState.volume = notification.body*1;
      console.log("group", _this.groupState.volume)
      discovery.emit('group-volume', {uuid: _this.uuid, state: _this.groupState});
      return;
    }
    ...
    xml2js.parseString(notification.body, function (err, result) {
      ...
      switch(eventType) {
        case 'TransportState':
          ...
          console.log(positionInfoTimeout, "is active, clearing")
        ...
        case 'Volume':
          ...
          if (!_this.ignoreVolumeEvents) {
            console.log("triggering state volume", _this.roomName)
...
Player.prototype.groupSetVolume = function (volumeLevel, callback) {
  ...
  for (var uuid in this.discovery.players) {
    ...
    totalVolume += player.state.volume;
    count++;

    actions.push(function (player) {
      return function (callback) {
        console.log(player.state.volume, totalVolume, count)
        var factor = player.state.volume / (totalVolume / count);
        if (isNaN( factor) || factor == 0) {
          factor = 1;
        } else if (factor > 3) {
          // We don't want a factor larger than 3 because it easily escalate
          factor = 3;
        }

        var roundFunction = (groupVolume > volumeLevel) ? Math.floor : Math.ceil;

        var memberVolume =roundFunction(factor * volumeLevel);
        console.log(player.roomName, memberVolume, factor, volumeLevel);
  ...
  groupVolume = Math.round(totalVolume / count);
  console.log("current group volume", groupVolume);
...
Player.prototype.replaceWithFavorite = function (favorite, callback) {
  ...
  player.getFavorites(function (success, favorites) {
    console.log("looking for favorite", favorite)
