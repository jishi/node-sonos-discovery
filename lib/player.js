...
Player.prototype.getDescriptionInfo = function(callback) {
    var _this = this;
    var req = http.request(_this.descriptorUrl, function(res) {
        var buffer = [];
        var result = {};
        var attrs = [ 'deviceType', 'friendlyName', 'manufacturer', 'modelNumber', 'modelDescription', 'modelName' ];
        res.setEncoding("utf-8");

        res.on('data', function(chunk) { buffer.push(chunk) });
        res.on('error', function(e) { _this.log.error(e); });
        res.on('end', function() {
            var saxParser = new EasySax();
            var nodeValue;
            saxParser.on('textNode', function (str, uq) { nodeValue = uq(str); });
            saxParser.on('endNode', function (elem) {
                if (attrs.indexOf(elem) != -1)
    ...
    req.on('error', function(e) { _this.log.error(e); });
    req.end();
}
