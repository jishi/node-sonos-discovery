...
function Player(roomName, descriptorUrl, uuid, discovery) {
  ...
  function updateTransportState(data) {
    ...
    // This always return an array, so first item is the actual parsed data
      console.log(data.CurrentTrackMetaData)
      var trackInfo = parseMetaDataXML(data.CurrentTrackMetaData)[0];
      _this.state.currentTrack = trackInfo;
      _this.state.currentTrack.type = isRadio ? 'radio' : 'track';

      //console.log('Enqueue metadata', data)
      if (isRadio && data['r:EnqueuedTransportURIMetaData']) {
        var radioInfo = parseMetaDataXML(data['r:EnqueuedTransportURIMetaData'])[0];
    ...
    if (data['r:NextTrackMetaData']) {
      ...
      trackInfo = parseMetaDataXML(data['r:NextTrackMetaData']);

      if (trackInfo.length > 0) {
        _this.state.nextTrack = trackInfo[0];
      }

      console.log("1", _this.state.currentTrack)
      console.log(2, _this.state.nextTrack)
      console.log(3, _this.state.currentTrack.radioShowMetaData)
