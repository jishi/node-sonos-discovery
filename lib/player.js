...
function parseMetaDataXML(xml, player) {
  ...
  saxParser.on('endNode', function (elem, unEntities, tagstart, getStringNode) {
    ...
    if (mapping[elem] === 'albumArtURI' && nodeValue.startsWith('/')) {
      ...
      currentItem['absoluteAlbumArtURI'] = 'http://' + player.address + ':1400' + nodeValue;
    } else {
      currentItem[mapping[elem]] = nodeValue || "";
    }
...
function Player(roomName, descriptorUrl, uuid, discovery) {
  ...
  function updateTransportState(data) {
    ...
    if (data.CurrentTrackMetaData) {
      ...
      var trackInfo = parseMetaDataXML(data.CurrentTrackMetaData, _this)[0];
    ...
    musicservices.tryGetHighResArt(_this.state.currentTrack.uri)
      .then((highResAlbumArtUrl) => {
        ...
        var absoluteUrl = 'http://' + _this.address + ':1400' + _this.state.currentTrack.albumArtURI;
        _this.state.currentTrack.absoluteAlbumArtURI = absoluteUrl;
        getPositionInfo();
      });

      musicservices.tryGetHighResArt(_this.state.nextTrack.uri)
       .then((highResAlbumArtUrl) => {
         _this.state.nextTrack.absoluteAlbumArtURI = highResAlbumArtUrl;
         getPositionInfo();
      }).catch(() => {
        var absoluteUrl = 'http://' + _this.address + ':1400' + _this.state.nextTrack.albumArtURI;
        _this.state.nextTrack.absoluteAlbumArtURI = absoluteUrl;
        getPositionInfo();
      });
  }
