...
Player.prototype.replaceWithFavorite = function (favorite, callback) {
  ...
  _this.getFavorites(function (success, favorites) {
    if (!success) {
    ...
    favorites.forEach(function (item) {
      if (item.title.toLowerCase() == decodeURIComponent(favorite).toLowerCase()) {
        ...
        if (item.uri.startsWith('x-sonosapi-stream:') || item.uri.startsWith('x-sonosapi-radio:') || item.uri.startsWith('pndrradio:')) {
          ...
          _this.setAVTransportURI(item.uri, item.metaData, function (success) {
            callback(success);
          });
        ...
        _this.removeAllTracksFromQueue(function (success) {
          if (!success) {
          ...
          _this.addURIToQueue(item.uri, item.metaData, function (success) {
            if (!success) {
            ...
            _this.setAVTransportURI(queueURI, '', function (success) {
              callback(success);
            });
          });
        });
...
Player.prototype.addPlaylistToQueue = function (playlistURI, callback) {
  var _this = this;
  _this.addURIToQueue(playlistURI, '', function (success) {
    if (!success) {
