...
Player.prototype.groupSetVolume = function (volumeLevel, callback) {
  ...
  // If prefixed with + or -
  console.log('current group volume', this.groupState.volume);
  if (/^[+\-]/.test(volumeLevel)) {
    volumeLevel = this.groupState.volume + volumeLevel*1;
  }
  ...
  this.groupState.volume = volumeLevel;

  // TODO: find a way to ignore events 

  var volumeSoap = SOAP.GroupVolume.format({volume: volumeLevel});
  var reqPath = '/MediaRenderer/GroupRenderingControl/Control';
  var soapAction = '"urn:schemas-upnp-org:service:GroupRenderingControl:1#SetGroupVolume"';
