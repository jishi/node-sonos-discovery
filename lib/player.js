...
function parsePlaylistXML(xml) {
  ...
  saxParser.on('endNode', function (elem, unEntities, tagstart, getStringNode) {
    ...
    if (!mapping[elem]) return;
    currentItem[mapping[elem]] = nodeValue||"";
    if(currentAttr!=null && currentAttr.length !==0){
...
Player.prototype.browse = function (objectID, startIndex, requestedCount, parseFn, callback) {
  ...
  this.soapAction('/MediaServer/ContentDirectory/Control', '"urn:schemas-upnp-org:service:ContentDirectory:1#Browse"', soap, function (success, res) {
    ...
    res.on('end', function () {
      ...
      response.items = parseFn(metaDataXML);
...
Player.prototype.getFavorites = function (callback) {
  this.browse("FV:2", null, null, parseMetaDataXML, function (success, result) {
    if (!success) {
      callback(false);
      return;
    }
...
Player.prototype.getPlaylists = function(callback){
  this.browse("SQ:", null, null, parsePlaylistXML,function (success, result) {
    if (!success) {
      callback(false);
      return;
    }
...
Player.prototype.loadPlaylist = function(playlistURI, callback){
  var player = this;
...
Player.prototype.replaceWithPlaylist = function(playlistURI, callback){
  var player = this;
  ...
  player.removeAllTracksFromQueue(function (success) {
    ...
    player.loadPlaylist(playlistURI, callback);
