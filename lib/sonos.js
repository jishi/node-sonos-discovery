...
Discovery.prototype.applyPreset = function (preset) {
  ...
  var coordinator = this.getPlayer(playerInfo.name);
  console.log("ungrouping", coordinator.roomName, "to prepare for grouping");
  coordinator.becomeCoordinatorOfStandaloneGroup(function () {
    ...
    for (var i = 1; i < preset.players.length; i++) {
      ...
      var player = _this.getPlayer(playerInfo.name);
      ...
      player.setVolume(playerInfo.volume);
    }
...
Player.prototype.setVolume = function (volumeLevel, forGroup) {
...
Player.prototype.mute = function (isMute, forGroup) {
    ...
    var req = http.request({
        ...
        headers: {
            'CONTENT-TYPE': 'text/xml; charset="utf-8"',
            'SOAPACTION': soapAction,
            'CONTENT-LENGTH': muteSoap.length
        }
    });
    req.write(muteSoap);
    req.end();
...
Player.prototype.setAVTransportURI = function (URI, metaData) {
...
Player.prototype.becomeCoordinatorOfStandaloneGroup = function (callback) {
  var req = http.request({
    ...
    headers: {
      ...
      'CONTENT-LENGTH': SOAP.BecomeCoordinatorOfStandaloneGroup.length
    }
  }, function (res) {
    console.log('STATUS: ' + res.statusCode);
    if (res.statusCode == 200) callback(this);
  });

  req.on('error', function(e) {
    console.log('problem with request: ' + e.message);
  });
  req.write(SOAP.BecomeCoordinatorOfStandaloneGroup);
  req.end();
}
