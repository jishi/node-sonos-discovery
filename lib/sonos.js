...
Discovery.prototype.applyPreset = function (preset) {
  ...
  var afterCoordinatorBreakout = function () {
    ...
    if (preset.uri)
      coordinator.setAVTransportURI(preset.uri);
...
Player.prototype.setAVTransportURI = function (uri, metaData, callback) {
  ...
  var body = SOAP.SetAVTransportURI.format({ URI: xmlEntities.encode(uri) || '', MetaData: xmlEntities.encode(metaData) || ''});
...
Player.prototype.addURIToQueue = function (uri, metaData, callback) {
  var xmlEntities = new XmlEntities();
  var body = SOAP.AddURIToQueue.format({ URI: xmlEntities.encode(uri) || '', MetaData: xmlEntities.encode(metaData) || ''});
