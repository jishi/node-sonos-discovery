...
Discovery.prototype.applyPreset = function (preset) {
  ...
  var afterCoordinatorBreakout = function () {
    ...
    // Lower the volume after the breakout
    coordinator.setVolume(coordinatorVolume);

    // If we have a uri in the preset, use that
    if (preset.favorite)
      coordinator.replaceWithFavorite(preset.favorite, afterCoordinatorPreparation);
    else if (preset.uri)
      coordinator.setAVTransportURI(preset.uri, afterCoordinatorPreparation);
    else 
      afterCoordinatorPreparation();
  }

  var afterCoordinatorPreparation = function () {
        ...
        return function (callback) {
          player.setVolume(volume, false, function (success) {
            ...
            if (player.currentTrackUri == streamUrl) {
              // If player already has the correct streamUrl, skip it!
  ...
  if (coordinator.coordinator.uuid == coordinator.uuid && preset.players.length > 1) {
    ...
    async.series(callbacks, afterCoordinatorBreakout);
